...
\brief      Headless CAN Packet routing test driven over a TCP host-link.
\details    Companion to test_can_packet_mode.cpp, which speaks over a serial
            port to a real device. This variant runs the same black-box CAN
            Packet assertions against a device exposed over a TCP host-link
            (e.g. a device emulator or a serial<->TCP bridge that carries the
            device's raw BST byte stream on a TCP port), so the round-trip can
            be exercised without dedicated serial hardware.

            The SDK ships only serial and loopback transports, so this test
            supplies a small TCP ITransport (below) and drives a normal session
            over it through the public openWithTransport() seam, proving CAN
            frames round-trip across the host link and that BEM still works
            while in the mode.

            ----------------------------------------------------------------
            Environment (self-skips when unset so the CI build stays green):
              - ACTISENSE_EMU_HOST : host-link address (default 127.0.0.1)
              - ACTISENSE_EMU_PORT : host-link TCP port (default 2000)
            Set ACTISENSE_EMU_ENABLE=1 to opt in. When the opt-in is absent, or
            the port cannot be connected, the fixture self-skips.

            For a meaningful CAN->serial pass the device's CAN interface must
            sit on a bus carrying traffic. The serial->CAN leg sends a
            distinctive burst that an external bus sniffer can witness.
