...
namespace Sdk
{
		...
		for (const uint8_t byte : data)
		{
			...
			case State::InFrame:
				...
				else
				{
					...
					else
					{
						...
						if (emitError)
						{
							emitError(ErrorCode::MalformedFrame, "BDTP frame exceeds maximum size");
						}
						++frames_dropped_;
						state_ = State::Idle;
						frame_buffer_.clear();
			...
			case State::InFrameGotDLE:
				...
				else if (byte == BdtpChars::STX)
				{
					...
					if (emitError)
					{
						emitError(ErrorCode::MalformedFrame, "BDTP frame aborted - new frame started");
					}
					++frames_dropped_;
					frame_buffer_.clear();
					state_ = State::InFrame;
				...
				else
				{
					...
					if (emitError)
					{
						emitError(ErrorCode::MalformedFrame, "Invalid BDTP escape sequence");
					}
					++frames_dropped_;
					state_ = State::Idle;
					frame_buffer_.clear();
	...
	std::string_view BdtpProtocol::stateName() const noexcept
	{
		...
		case State::GotDLE:        return "GotDLE";
		case State::InFrame:       return "InFrame";
		case State::InFrameGotDLE: return "InFrameGotDLE";
	...
	void BdtpProtocol::processCompletedFrame(
		MessageEmitter emitMessage,
		ErrorEmitter emitError)
	{
	...
	bool BdtpProtocol::parseBstFromFrame(
		ConstByteSpan frameData,
		BstDatagram& outDatagram,
		std::string& outError) const
	{
		...
		outDatagram.data.assign(
			frameData.begin() + 2,
			frameData.begin() + 2 + storeLength);
