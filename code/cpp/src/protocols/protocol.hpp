...
namespace Actisense
{
	namespace Sdk
	{
		...
		public:
			 ...
			 *******************************************************************************/
			virtual std::size_t parse(ConstByteSpan data, MessageEmitter emitMessage,
			 ...
			 *******************************************************************************/
			virtual bool encode(std::string_view messageType, ConstByteSpan payload,
								std::vector<uint8_t>& outFrame, std::string& outError) = 0;
			...
			correlate(const ParsedMessageEvent& msg) const {
				(void)msg;
				return std::nullopt;
			}
