...
namespace Actisense
{
	namespace Sdk
	{
		 ...
		 *******************************************************************************/
		[[nodiscard]] inline bool decodeTxPgnEnableListF1Response(
			...
			std::string& outError)
		{
			...
			if (response.entryCount > kTxPgnEnableListF1PgnsPerMessage) {
				outError = "Entry count " + std::to_string(response.entryCount) +
				           " exceeds max per message " + std::to_string(kTxPgnEnableListF1PgnsPerMessage);
				return false;
			}
			...
			if (response.isPgnListMessage()) {
				...
				const std::size_t expectedSize = kTxPgnEnableListF1ResponseHeaderSize +
				...
				if (data.size() < expectedSize) {
					outError = "Tx PGN Enable List F1 response too short for " +
					           std::to_string(response.entryCount) + " rate/priority pairs: expected " +
					           std::to_string(expectedSize) + " bytes, got " +
					           std::to_string(data.size());
					return false;
				}
		 ...
		 *******************************************************************************/
		inline void encodeTxPgnEnableListF1GetRequest(
		...
		/**************************************************************************/ /**
		 ...
		 \return     True on success, false on error
		 *******************************************************************************/
		[[nodiscard]] inline bool encodeTxPgnEnableListF1SetPgns(
			uint8_t messageIndex,
		...
		[[nodiscard]] inline bool encodeTxPgnEnableListF1SetRatePriority(
			uint8_t messageIndex,
			const std::vector<std::pair<uint8_t, uint8_t>>& ratePriority,
			std::vector<uint8_t>& outData,
			std::string& outError)
		{
			...
			if (ratePriority.size() > kTxPgnEnableListF1PgnsPerMessage) {
				outError = "Too many entries for single message: " + std::to_string(ratePriority.size()) +
				           " exceeds max " + std::to_string(kTxPgnEnableListF1PgnsPerMessage);
				return false;
			}
		 ...
		 *******************************************************************************/
		[[nodiscard]] inline std::string formatTxPgnEnableListF1(
			const TxPgnEnableListF1Response& response)
		{
			std::string result;
			result.reserve(128);
			...
			if (response.isPgnListMessage()) {
				...
				for (std::size_t i = 0; i < response.ratePriority.size(); ++i) {
					result += "  [" + std::to_string(i) + "] Rate=" +
					          std::to_string(response.ratePriority[i].first) +
