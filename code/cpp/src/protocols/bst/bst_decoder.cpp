...
namespace Actisense
{
	namespace Sdk
	{
		...
		static constexpr std::size_t kBst93MinLength =
			13; /* P,PDUS,PDUF,DP,D,S,T0-T3,DL = 13 + data */
		...
		static constexpr std::size_t kBstD0MinLength =
			11; /* D,S,PDUS,PDUF,DPP,C,T0-T3 = 11 minimum */
		...
		BstDecodeResult BstDecoder::decode(ConstByteSpan data) const {
			...
			switch (bstId) {
				...
				default:
					result.error =
						"Unsupported BST ID: 0x" + std::to_string(static_cast<unsigned>(bstId));
					break;
			}
		...
		std::optional<Bst93Frame> BstDecoder::decode93(ConstByteSpan data,
			...
			frame.data.assign(payload.begin() + kBst93OffData,
							  payload.begin() + kBst93OffData + dataLen);

			return frame;
		}

		std::optional<Bst94Frame> BstDecoder::decode94(ConstByteSpan data,
													   std::string& outError) const {
			...
			frame.data.assign(payload.begin() + kBst94OffData,
							  payload.begin() + kBst94OffData + dataLen);

			return frame;
		}

		std::optional<Bst95Frame> BstDecoder::decode95(ConstByteSpan data,
													   std::string& outError) const {
			/* Minimum: ID(1) + Len(1) + header(6) = 8 bytes */
			...
			frame.data.assign(payload.begin() + kBst95OffData,
		...
		std::optional<BstD0Frame> BstDecoder::decodeD0(ConstByteSpan data,
			...
			/* Control byte: bits 0-1 = MsgType, 2 = Spare, 3 = Direction, 4 = Source, 5-7 = FP
			 * SeqId */
		...
		bool BstEncoder::encode94(const Bst94Frame& frame, std::vector<uint8_t>& outData,
								  std::string& outError) const {
		...
		bool BstEncoder::encodeD0(const BstD0Frame& frame, std::vector<uint8_t>& outData,
								  std::string& outError) const {
			...
			/* Control byte */
			const uint8_t ctrl = (static_cast<uint8_t>(frame.messageType) & 0x03) |
