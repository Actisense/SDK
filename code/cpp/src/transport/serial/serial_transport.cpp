...
namespace Sdk
{
	...
	SerialTransport::SerialTransport()
		: readBuffer_(kDefaultReadBufferSize)
	{
	}
	...
	void SerialTransport::asyncOpen(
		const TransportConfig& config,
		std::function<void(ErrorCode)> completion)
	{
		if (config.kind != TransportKind::Serial)
		{
			if (completion) completion(ErrorCode::InvalidArgument);
			return;
		}
	...
	void SerialTransport::close()
	{
		...
		/* Clear pending operations */
		{
			...
			while (!pendingRecvs_.empty())
			{
				...
				if (op.completion) op.completion(ErrorCode::TransportClosed, 0);
				pendingRecvs_.pop();
			}
	...
	void SerialTransport::asyncSend(
		ConstByteSpan data,
		SendCompletionHandler completion)
	{
		if (!isOpen())
		{
			if (completion) completion(ErrorCode::NotConnected, 0);
			return;
		}
	...
	void SerialTransport::asyncRecv(
		ByteSpan buffer,
		RecvCompletionHandler completion)
	{
		...
		/* Check if data already available */
		{
			...
			if (available > 0)
			{
				...
				if (completion) completion(ErrorCode::Ok, toRead);
				return;
			}
		}
	...
	std::size_t SerialTransport::readSync(uint8_t* buffer, std::size_t maxBytes, unsigned timeoutMs)
	{
		...
		if (!ReadFile(handle_, buffer, static_cast<DWORD>(maxBytes), &bytesRead, &readOverlapped_))
		{
	...
	std::size_t SerialTransport::writeSync(const uint8_t* data, std::size_t size)
	{
		...
		if (!WriteFile(handle_, data, static_cast<DWORD>(size), &bytesWritten, &writeOverlapped_))
		{
			if (GetLastError() == ERROR_IO_PENDING)
			{
	...
	ErrorCode SerialTransport::configurePort(const SerialTransportConfig& config)
	{
		...
		/* Set buffer sizes */
		SetupComm(handle_, 
			static_cast<DWORD>(config.readBufferSize),
