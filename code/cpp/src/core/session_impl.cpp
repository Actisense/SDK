...
namespace Sdk
{
		...
		SendCompletion completion)
	{
		if (!isConnected())
		{
			if (completion) completion(ErrorCode::NotConnected);
			return;
		}
	...
	void SessionImpl::sendBemCommand(
	...
	void SessionImpl::getOperatingMode(
		std::chrono::milliseconds timeout,
		BemResponseCallback callback)
	{
		BemCommand cmd;
	...
	void SessionImpl::setOperatingMode(
		uint16_t mode,
		std::chrono::milliseconds timeout,
		BemResponseCallback callback)
	{
	...
	void SessionImpl::receiveThreadFunc()
	{
		...
		while (running_ && isConnected())
		{
			...
			transport_->asyncRecv(
				buffer,
				[this, &buffer](ErrorCode code, std::size_t bytesRead) {
	...
	std::unique_ptr<SessionImpl> createSerialSession(
